...
float3						new_get_ray_cam(__local t_scen *scene, const int x, const int y)
{
	float3					cam_ray = 0;
	float					ratio = (float)scene->win_w / (float)scene->win_h;

	cam_ray.x = ((2 * ((x + 0.5) / scene->win_w)) - 1) * ratio * (tan(radians(scene->cameras[scene->active_cam].fov / 2)));
	cam_ray.y = ((1 - (2 * ((y + 0.5) / scene->win_h))) * tan(radians(scene->cameras[scene->active_cam].fov / 2)));
	cam_ray.z = 1;
/*  rotation XYZ
	cam_ray.x = xx * cos(yaw) * cos(roll) + yy * cos(yaw) * -sin(roll) + sin(yaw);
	cam_ray.y = xx * (-sin(pitch) * -sin(yaw) * cos(roll) + cos(pitch) * sin(roll)) + yy * (-sin(pitch) * -sin(yaw) * -sin(roll) + cos(pitch) * cos(roll)) + cos(yaw) * -sin(pitch);
...
calcul simplifié
	...
	cam_ray.z = xx * -sin(yaw) + yy * cos(yaw) * sin(pitch) + cos(yaw) * cos(pitch);
*/
	cam_ray = rotat_zyx(cam_ray, scene->cameras[scene->active_cam].pitch, scene->cameras[scene->active_cam].yaw, 0);
	return(normalize(cam_ray));
}
